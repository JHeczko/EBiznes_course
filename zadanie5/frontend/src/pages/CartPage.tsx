...
function CartPage({basket, setBasket}: CartPageProps) {
    ...
    const handleQuantityChange = async (product: Product, userID: number, newQuantity: number) => {
        ...
        try {
            ...
            if (status === 200) {
                // Używamy prevBasket, żeby uniknąć problemów z asynchronicznością
                setBasket((prevBasket) =>
                    prevBasket.map((item) => {
                        // Sprawdź w konsoli czy te dwa ID na pewno do siebie pasują!
    ...
    const deleteItem = async (product: Product, userID: number) => {
        ...
        try{
            ...
            if (status === 200) {
                ...
                toast.success(`Deleted "${product.name}" for cart`);
            }
        }catch(error) {
    ...
    const handlePayment = async (userID: number) => {
        try{
            const [data, status] = await addPayment(userID)
            if (status === 200){
                const status_clear = await clearBasket(userID)
                if (status_clear){
                    setBasket([])
                }
    ...
    return (
        <main className="cart-page">
                        ...
                        <div className="cart-item" key={item.id}>
                            ...
                            <div className="item-lower">
                                <div className="control-quantity">
                                    <div className="quantity-control-button" onClick={() => handleQuantityChange(item.product, item.user_id, item.quantity - 1)}>-</div>
                                    {/*<input type="number" value={item.quantity} className="quantity-input" min="0"></input>*/}
                                    <h3>{item.quantity}</h3>
                                    <div className="quantity-control-button" onClick={() => handleQuantityChange(item.product, item.user_id, item.quantity+1)}>+</div>
                                    <div className="delete-button" onClick={() => deleteItem(item.product, item.user_id)}><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18"></path><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"></path><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg></div>
                                </div>
            ...
            {
                basket.length !== 0 ? (<div className="checkout-button" onClick={()=>{handlePayment(1)}}>Checkout</div>) : null
            }
        </main>
    )
}
